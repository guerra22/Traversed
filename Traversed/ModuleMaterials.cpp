...
bool ModuleMaterials::Import(const char* file_path, MaterialData* newMaterial)
{
	...
	if (file_path != nullptr)
	{
		...
		char* data = nullptr;
		uint file_size = App->filesystem->Load(file_path, &data);

		if (file_size > 0)
		{
			ILenum type = IL_TYPE_UNKNOWN;

			if (App->filesystem->GetFileExtension(file_path) == "png" || App->filesystem->GetFileExtension(file_path) == "PNG")
			{
				type = IL_PNG;
			}

			if (App->filesystem->GetFileExtension(file_path) == "dds" || App->filesystem->GetFileExtension(file_path) == "DDS")
			{
				type = IL_DDS;
			}
...
uint ModuleMaterials::CreateTexture(const void* data, uint width, uint height, int internal_format, uint format, uint target, int filter_type, int filling_type)
{
	uint textureId = 0;
