...
#pragma comment(lib, "External/PhysFS/lib/physfs.lib")
...
size_t AssimpWrite(aiFile* file, const char* data, size_t size, size_t chunks)
{
	...
	if (ret == -1)
		LOGGING("File System error while WRITE via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpRead(aiFile* file, char* data, size_t size, size_t chunks)
{
	...
	if (ret == -1)
		LOGGING("File System error while READ via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpTell(aiFile* file)
{
	...
	if (ret == -1)
		LOGGING("File System error while TELL via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpSize(aiFile* file)
{
	...
	if (ret == -1)
		LOGGING("File System error while SIZE via assimp: %s", PHYSFS_getLastError());
...
void AssimpFlush(aiFile* file)
{
	if (PHYSFS_flush((PHYSFS_File*)file->UserData) == 0)
		LOGGING("File System error while FLUSH via assimp: %s", PHYSFS_getLastError());
}
...
aiReturn AssimpSeek(aiFile* file, size_t pos, aiOrigin from)
{
	...
	if (res == 0)
		LOGGING("File System error while SEEK via assimp: %s", PHYSFS_getLastError());
...
void AssimpClose(aiFileIO* io, aiFile* file)
{
	if (PHYSFS_close((PHYSFS_File*)file->UserData) == 0)
		LOGGING("File System error while CLOSE via assimp: %s", PHYSFS_getLastError());
}
